...
class Decorations(Transform):
    ...
    def apply(self):
        if header_nodes := self.generate_header():
            decoration = self.document.get_decoration()
            header = decoration.get_header()
            header.extend(header_nodes)
        if footer_nodes := self.generate_footer():
            decoration = self.document.get_decoration()
    ...
    def generate_footer(self):
        ...
        if (
            not settings.generator
            and not settings.datestamp
            and not settings.source_link
            and not settings.source_url
        ):
            return None
        text = []
        if settings.source_link and settings._source or settings.source_url:
            source = settings.source_url or utils.relative_path(settings._destination,
                                                 settings._source)
            text.extend([
        ...
        if settings.datestamp:
            ...
            text.append(nodes.Text(f'Generated on: {datestamp}' + '.\n'))
...
class StripClassesAndElements(Transform):
    ...
    def check_classes(self, node):
        if not isinstance(node, nodes.Element):
            return False
        return any(
            class_value in self.strip_elements
...
class SmartQuotes(Transform):
    ...
    def apply(self):
        ...
        if lc_smartquotes := self.document.settings.smartquotes_locales:
